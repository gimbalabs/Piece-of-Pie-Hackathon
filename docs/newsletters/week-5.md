# 🥧 Piece of Pie Hackathon— Weekly Update

---

## 🧭 State of the Hackathon

* **Week:** 5 of 12
* **Active Builders:** 17
* **Total Updates This Week:** 15

---

## 🔥 What Happened This Week

- 🚀 **First user signups for projects**
- 🧑‍💻 **Live coding sessions hosted**
- 📝 **Builder blogs and tutorials shared**
- 🐦 **Weekly update tweets highlight user wins**
- 🤝 **Mentor office hours launched**
- 🛠️ **Bug bounties announced**

---

## 👷 Builder Updates

* 15 builders/projects posted updates
* Several projects gained first real users
* Tutorials and guides published in repo

---

## ⏰ Deadlines & Reminders

* Weekly update due: Sunday 2026-05-24 1200 UTC
* Weekly update tweets due each Sunday 1200 UTC
* Final presentations: **Week 12** - 2026-07-05

---

## 🔄 Changes / Announcements

* Mentor office hours every Wednesday
* Bug bounty program now live
* Submit your tutorials for extra feedback

---

## 💬 Feedback Loop

* Open requests for feedback:
  * [Tutorials and blogs](https://www.gimbalabs.com/piece-of-pie)
  * [Feedback channel in Discord](https://discord.com/channels/767416282198835220/1493276804314828871)
* To request or give feedback, use the Discord channel

---

## 🧠 Focus for Next Week

**Theme:** *User → Traction*

- 👥 Grow your user base
- 📝 Document user stories
- 🧑‍💻 Share your process and learnings

This week is about traction and outreach.

> Users are the best validation.

**Prompt:**

- How did you get your first user?
- What did you learn from them?
- What’s your plan to grow?

---

## 🐦 Tweets of the Week

* 

---

## 🔗 Quick Links

* Main page: [https://www.gimbalabs.com/piece-of-pie](https://www.gimbalabs.com/piece-of-pie)
* Project list: see repo
* Submit your update: [@gimbalabs](https://x.com/gimbalabs)
* Feedback channel: [#piece-of-pie-feedback-requests](https://discord.com/channels/767416282198835220/1493276804314828871)
* Builder discussion channel: [#piece-of-pie-hackathon](https://discord.com/channels/767416282198835220/1493275589325492234)

---

## 🍰 Reminder

Traction matters. Users matter. Keep building.

---
