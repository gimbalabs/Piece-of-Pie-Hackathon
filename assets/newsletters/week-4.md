# 🥧 Piece of Pie Hackathon— Weekly Update

---

## 🧭 State of the Hackathon

* **Week:** 4 of 12
* **Active Builders:** 17
* **Total Updates This Week:** 16

---

## 🔥 What Happened This Week

- 🏗️ **MVPs refined** — new features shipped
- 🧪 **First user tests conducted**
- 📝 **Builder retrospectives posted**
- 🐦 **Weekly update tweets highlight pivots**
- 🤝 **Cross-team code reviews started**
- 🛠️ **Admin dashboard improvements**

---

## 👷 Builder Updates

* 16 builders/projects posted updates
* User feedback led to several pivots
* Teams sharing code and resources

---

## ⏰ Deadlines & Reminders

* Weekly update due: Sunday 2026-05-17 1200 UTC
* Weekly update tweets due each Sunday 1200 UTC
* Final presentations: **Week 12** - 2026-07-12

---

## 🔄 Changes / Announcements

* New code review channel in Discord
* Builder retrospectives now part of weekly update
* Reminder: submit blockers for support

---

## 💬 Feedback Loop

* Open requests for feedback:
  * [Retrospectives](https://www.gimbalabs.com/piece-of-pie)
  * [Feedback channel in Discord](https://discord.com/channels/767416282198835220/1493276804314828871)
* To request or give feedback, use the Discord channel

---

## 🧠 Focus for Next Week

**Theme:** *Iterate → Improve*

- 🔄 Iterate on your MVP
- 🧠 Apply user feedback
- 📝 Document learnings and next steps

This week is about continuous improvement.

> Small changes, big impact.

**Prompt:**

- What did you improve this week?
- What feedback was most valuable?
- What’s your next milestone?

---

## 🐦 Tweets of the Week

* 

---

## 🔗 Quick Links

* Main page: [https://www.gimbalabs.com/piece-of-pie](https://www.gimbalabs.com/piece-of-pie)
* Project list: see repo
* Submit your update: [@gimbalabs](https://x.com/gimbalabs)
* Feedback channel: [#piece-of-pie-feedback-requests](https://discord.com/channels/767416282198835220/1493276804314828871)
* Builder discussion channel: [#piece-of-pie-hackathon](https://discord.com/channels/767416282198835220/1493275589325492234)

---

## 🍰 Reminder

Iterate. Improve. Share. Repeat.

---
